...
var MONGOSTORE = module.exports = function MongoStore(options, callback) {
  ...
  function getCollectionCore(db, callback) {
    db.collection(options.collection || _defaults.collection, function (err, col) {
      _collection = col;

      if (err || !connectionQueue.length) {
        callback(err, col);
        return;
      }
      
      // fire pending callbacks in order.
      //
  ...
  if (options.reapInterval !== -1) {
    ...
    var reap_interval = setInterval(function () {
      self.getCollection(function(err, c) {
         c.remove({expires: {'$lte': Date.now()}}, function () { });
      });
...
MONGOSTORE.prototype.get = function (sid, cb) {
  cb = _default(cb);
  this.getCollection(function(err, c){
      if (err) return cb(err);
      c.findOne({_id: sid}, function (err, data) {
        try {
          if (data) {
...
MONGOSTORE.prototype.set = function (sid, sess, cb) {
  ...
  this.getCollection(function(err, c){
      if (err) return cb(err);
      c.update({_id: sid}, update, {upsert: true}, function (err, data) {
...
MONGOSTORE.prototype.getCollection = function (callback) {
    // there are cases where a session method can be
    // called before the collection is ready.
    // in that case, queue the request.
    if (callback) {
      if (_collection) {
